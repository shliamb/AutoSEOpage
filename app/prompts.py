...
tools_3 = json.dumps([
    {
        "function_declarations": [
            {
                "name": "mainset",
                "description": "На основе данных определи и собери подходящие данные в json. Ответ должен быть четким, структурированным и без лишней информации",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "goals": {
                            "description": "Определи что хочет получить пользователь на этой странице, какова его цель",
                            "type": "string"
                        },
                        "fears": {
                            "description": "Выяви его скрытые страхи и возражения, конкретные примеры страхов для темы которые могут влиять на его решения и действия",
                            "type": "string"
                        },
                        "benefit": {
                            "description": "Предложи варианты, как превратить эти страхи в выгоды в тексте, что бы дать ему то чего он хочет",
                            "type": "string"
                        }
                    },
